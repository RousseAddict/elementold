...
class CurlFetcher {
    ...
    // whole duration. With no ceiling, a burst of requests (e.g. one download
    // per visible thumbnail, re-fired on every /sync-driven table reload) made
    // GCD spin up threads without limit until the pool was exhausted: later
    // requests then starved (nothing past the first screen ever loaded) and all
    // those blocked threads pinned the CPU (the device warmed up). An
    // NSOperationQueue with a fixed width lets excess requests WAIT in the queue
    // without consuming a thread, capping how many blocking curl calls run at
    // once. 6 leaves room for the perpetual /sync long-poll (~1 slot) plus a
    // screenful of thumbnails. (A serial queue was rejected earlier because the
    // 40s /sync would block everything behind it — bounded-concurrent keeps that
    // fix while removing the thread explosion.) curl_easy_init needs no
    // serialization here: curl_global_init (below) is the only non-thread-safe
    // call and Swift's `static let` already runs it exactly once before any
    // worker touches curl.
    private static let curlQueue: OperationQueue = {
    ...
    // Path to the app-bundled Mozilla CA root bundle. iOS has no system trust
    // store our vendored OpenSSL can read, so cert verification only works if we
    ...
    private static func applyTLS(_ h: CurlHandle!) {
        curl_bridge_set_ssl_verify(h)
        if let path = caBundlePath {
            path.withCString { curl_bridge_set_ca_bundle(h, $0) }
        }
    }
    ...
    static func fetchData(url: String,
        ...
        retain(fetcher)
        CurlFetcher.curlQueue.addOperation {
    ...
    // PUT url with JSON body + custom headers -> Data on a background thread;
    // completion on the main thread. Used for Matrix event sends/state updates.
    static func putData(url: String,
                        headers: [String: String],
                        body: Data,
                        timeout: Int = 30,
                        completion: @escaping (Data?) -> Void) {
    ...
    private func syncFetchData(url: String, headers: [String: String], timeout: Int) -> Data? {
        ...
        // Masking non-2xx here previously turned real server errors into a
    ...
    private func syncPutData(url: String, headers: [String: String], body: Data, timeout: Int) -> Data? {
        _ = CurlFetcher.curlGlobalInit
        let h = curl_bridge_init()
        defer { curl_bridge_cleanup(h) }
        ...
        body.withUnsafeBytes { (raw: UnsafeRawBufferPointer) in
            curl_bridge_set_put_body(h, raw.baseAddress, CLong(body.count))
        }
